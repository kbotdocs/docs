...
category: 자습서
milestone:
  prev: learn/js/basic/9-conditionals
  next: learn/js/basic/11-function
---
...
<Section>
  ...
  1부터 100까지의 메시지를 보낸다는 단순한 동작임에도 불구하고 이를 구현하기 위해 100줄의 코드를 작성하게 됩니다. 반면, 곧 알아보게 될 반복문인 `for` 문을 사용하면 매우 간결한 코드로 같은 동작을
  수행하게 할 수 있습니다.
...
<Section>
  ...
  <Noti type="warning">
    Rhino 엔진(메신저봇R 0.7.40-alpha.02 이하 버전)에서 `const` 키워드의 동작이 정상적이지 못합니다(함수 스코프를 가짐). 그러므로 해당 버전을 사용하는 경우 `const` 키워드의 사용에 유의하세요.
  </Noti>
...
<Section>
  ...
  `break`는 `switch` 문에서만 쓰이지 않습니다. `for` 문, `while` 문과 같은 반복문에도 사용됩니다. 반복문에서 사용될 경우 해당 반복문을 탈출하고 반복문 다음의 코드를 수행하게 됩니다.
  아래는 `break`를 활용한 예제입니다.
...
<Section>
  ...
  `continue`는 반복문 내에서 사용하는 키워드로, 반복문 내의 코드를 수행하기를 중지하고 증감식 수행 단계로 건너뜁니다. `break`와 다르게 반복을 탈출하지 않습니다. 아래는 `continue`를 사용한
  예제입니다.
...
<Section>
  ...
  `java.lang.Thread.sleep()`은 입력된 시간만큼 코드의 흐름을 멈춥니다. 밀리초(1밀리초 = 1/1000초) 단위를 사용하기 때문에 1초 간 멈추기 위해서는
  `java.lang.Thread.sleep(1000)`으로 사용해야 합니다.
...
<Section>
  ...
  <Details summary={"모범답안"}>
    ...
    function onCommand(msg) {
      if (msg.command === "timer") {
        let sec = Number(msg.args[0]); // 사용자가 입력한 시간(초)를 숫자로 형변환
