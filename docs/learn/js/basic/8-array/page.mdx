...
category: 자습서
milestone:
  prev: learn/js/basic/7-string
  next: learn/js/basic/9-conditionals
---
...
<Section>
  ...
  ~~~js
  let arr = [1, "hello", true];
  msg.reply(arr.toString()); // 메시지: "1,hello,true"
  ~~~
</Section>
<Section>
  ## 접근/할당
  ...
  <Noti type={"info"}>
    ...
    특히, 메신저봇R 0.7.36a 이상 버전을 사용할 경우, 아래와 같은 방법을 사용할 수도 있습니다.
...
<Section>
  ...
  arr.push("grape");
  msg.reply(arr.toString()); // 메시지: "apple,banana,orange,grape"
  ~~~
  ...
  let arr = ["apple", "banana", "orange"];
  arr.unshift("grape");
  msg.reply(arr.toString()); // 메시지: "grape,apple,banana,orange"
  ~~~
  ...
  msg.reply(arr.toString()); // 메시지: "apple,grape,banana,orange"
  ~~~
</Section>
<Section>
  ## 배열의 요소 제거하기
  ...
  let arr = ["apple", "banana", "orange"];
  arr.pop();
  msg.reply(arr.toString()); // 메시지: "apple,banana"
  ~~~
  ...
  let arr = ["apple", "banana", "orange"];
  arr.shift();
  msg.reply(arr.toString()); // 메시지: "banana,orange"
  ~~~
  ...
  msg.reply(arr.toString()); // 메시지: "apple,orange"
  ~~~
</Section>